...
### Mettre la variable poids_sondage après la variable date_naissance À FAIRE
...
### `data.table` {.unnumbered .unlisted}
```{r, warning=FALSE, message=FALSE, results='hide'}
varAvant <- c( colnames(donnees_datatable)[1 : which("date_naissance" == colnames(donnees_datatable))], "poids_sondage" )
setcolorder(donnees_datatable, c(varAvant, setdiff(colnames(donnees_datatable), varAvant)))
```
