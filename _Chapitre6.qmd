...
## Création et suppressions de variables 
...
Comme en `dplyr`, la création de colonnes en DuckDB passe par `mutate`, et la suppression par `select`. 

```{r, warning=FALSE, message=FALSE}
# Création de la colonne note2
requete_duckdb %>% 
  mutate(note2 = as.numeric(note_contenu) / 20 * 5, 
         note_formateur2 = as.numeric(note_formateur) / 20 * 5) %>% 
  select(note2, note_formateur2)
...
## Formater les modalités pour une variable catégorielle
::: {.panel-tabset group="language"}
...
## `arrow/duckdb` {.unnumbered .unlisted}
Le code `sexef_format[Sexef]` utilise l'indexation de `rbase`, ce n'est pas vraiment un code `dplyr`. Il ne fonctionne pas en DuckDB.

```{r, warning=FALSE, message=FALSE}
# requete_duckdb %>% 
#   mutate(sexef = sexef_format[Sexef],
#          cspf = cspf_format[CSPF])
```
...
Préférer `case_match` quand il s'agit de valeurs déterminées, connues à l'avance (plus rapide).
...
{r, warning=FALSE, message=FALSE}
requete_duckdb %>%
  mutate(civilite = case_when(Sexef == "2" ~ "Mme",
                              Sexef == "1" ~ "M",
                              TRUE        ~ "Inconnu")
)
```
...
requete_duckdb %>% 
  mutate_at(enDate, ~ as.Date(strptime(.,'%d/%m/%Y'))) %>% # strptime est une fonction duckdb
  mutate(mois=as.character(month(Date_entree)),
         mois=ifelse(str_length(mois)<2, paste0("0", mois), mois),
         mois2 = str_pad(mois,2,side="left",pad="0")) %>% 
  select(mois, mois2, Date_entree)
